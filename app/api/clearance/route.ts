...
async function fetchStore(storeId: StoreId, province: string): Promise<ClearanceItem[]> {
  if (storeId === 'homedepot') {
    ...
    return ((data.searchReport?.products ?? data.products ?? data.results ?? []) as Record<string, unknown>[]).flatMap(p => {
  ...
  if (storeId === 'walmart') {
    ...
    return ((data.items ?? data.products ?? data.results ?? []) as Record<string, unknown>[]).flatMap(p => {
      ...
      return [{ id: `wm-${p.itemId ?? p.id}`, storeId: 'walmart' as const, storeLocation: store,
        ...
        originalPrice: orig || curr, clearancePrice: curr,
        discountPercent: orig > curr ? Math.round((1 - curr / orig) * 100) : 0,
        imageUrl: imgs[0] ? String((imgs[0] as Record<string, unknown>).url ?? '') : undefined,
  ...
  if (storeId === 'canadiantire') {
    ...
    return ((data.products ?? data.results ?? []) as Record<string, unknown>[]).flatMap(p => {
      ...
      const orig = Number(wasPrice.value ?? 0)
...
export async function GET(req: NextRequest) {
  ...
  try {
    const allItems = await fetchStore(storeId, province)
